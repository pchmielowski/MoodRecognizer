...
BOOST_AUTO_TEST_CASE(likelihood_2gaussComponents1d) {
	...
	double result0 = ubm.logLikelihood(x, 0);
	double result1 = ubm.logLikelihood(x, 1);
...
BOOST_AUTO_TEST_CASE(likelihood_1gaussComponent2d) {
	Ubm ubm;
	ubm.means_ = (cv::Mat_<double>(2, 1) << 7.5, .3);
	ubm.numGaussComponents_ = 1;
	int numDimensions = 2;
	vector<Mat> covs;
	covs.push_back((cv::Mat_<double>(2, 2) << 2.0, 0.0, 0.0, 100.2));
	ubm.createNormalDistribution(
		numDimensions, covs);
	Mat x = (cv::Mat_<double>(2, 1) << 2, -23);
...
BOOST_AUTO_TEST_CASE(likelihood_2gaussComponents2d) {
	...
	int numDimensions = 2;
	vector<Mat> covs;
	covs.push_back((cv::Mat_<double>(2, 2) << 4, 0.0, 0.0, 4));
	covs.push_back((cv::Mat_<double>(2, 2) << 285.0, 0.0, 0.0, 48));
	ubm.createNormalDistribution(
		numDimensions, covs);
	Mat x = (cv::Mat_<double>(2, 1) << 66, -66);
